import { MAX_EXTENSIONS_PER_REQUEST, OMAHA_JSON_PREFIX, UPDATE_SERVICES } from './constants';
...
import type { OmahaResponse, ProtocolVersion, ServiceId } from './types';
...
const craftRequest = async (apps: App[], version: ProtocolVersion, env: Env) => {
    const request = requestTemplateFor(version);
    const browserVersion = await Chromium.getRandomVersion(env);
