        ...
        app: response.app && await Promise.all(response.app.map(async (app) => {
            ...
            if (updatecheck?.status === 'ok') {
                ...
                updatecheck.urls.url = [
                    { codebase: await ExtensionProxy.wrap(url.toString(), env) },
                ];
            }
...
const handleRedirect = (data: OmahaResponse) => {
    ...
    const firstUrl = updateCheck?.status === 'ok' ? updateCheck.urls.url[0] : undefined;

    if (firstUrl && 'codebase' in firstUrl) {
        return Util.respond(302, 'Found', {
            location: firstUrl.codebase,
        });
    }
