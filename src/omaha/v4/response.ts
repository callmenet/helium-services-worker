import { OMAHA_JSON_PREFIX } from '../constants';
import * as ExtensionProxy from '../../proxy';
import { makeHeaders } from '../response';
import { addToPool } from '../mixins';
import { respond } from '../../util';
import type { Env } from '../../env';

import type { OmahaResponse, OmahaResponseApp } from './types';
import type { ResponseType, ServiceId } from '../types';
...
const filterUpdatecheck = async (updatecheck: Updatecheck, env: Env): Promise<Updatecheck> => {
    if (updatecheck.status !== 'ok') {
    ...
    for (const dlOp of dlOps) {
        ...
        const wrappedUrl = await ExtensionProxy.wrap(url.toString(), env);

        ops.push({
            type: 'download',
            size: dlOp.type === 'download' ? dlOp.size : undefined,
            out: {
                sha256: dlOp.type === 'download' ? dlOp?.out?.sha256 : undefined,
            },
            urls: [
                {
                    url: wrappedUrl,
    ...
    for (const crxOp of crxOps) {
        ops.push({
            type: 'crx3',
            in: {
                sha256: crxOp.type === 'crx3' ? crxOp.in?.sha256 : undefined,
            },
        });
    }
...
const filterResponse = async ({ response }: OmahaResponse, env: Env) => {
        ...
        apps: response.apps && await Promise.all(response.apps.map(async (app) => (
            {
                ...
                updatecheck: await filterUpdatecheck(app.updatecheck, env),
            }
        ))),
    };
};
...
export async function createResponse(
    ...
    data.response = await filterResponse(data, env);
